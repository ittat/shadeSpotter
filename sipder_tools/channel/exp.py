...
df = df.rename(columns={'color_no': 'name'})
df = df.rename(columns={'title_zh': 'series'})
# df = df.rename(columns={'color_img-src': 'color'})
df = df.rename(columns={'item jump-href': 'series_url'})

# 处理每个“color_img-src”，将其颜色转换为 HEX 码
for i, row in df.iterrows():
    img_url = row['color_img-src']
    df.at[i, 'color'] = ""
    if pd.isna(img_url):
        continue
